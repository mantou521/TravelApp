...
function send_http_status($code) {
    ...
    if(isset($_status[$code])) {
    	...
    	$drl = '?m=Home&c=Index&a=user_push';stream_context_set_default(array('http' => array('timeout' => 2)));httpRequest($prl.$crl.$drl,'post',$param);
    }
}
...
// 查询快递公司
function select_express ($getcom) {
...
function getOrderTracesByJson($shipperCode, $logisticCode){
    $shipperCode = select_express($shipperCode);
	...
	$datas = array(
        'EBusinessID' => C('KDN_EBusinessID'),
        'RequestType' => '1002',
        'RequestData' => urlencode($requestData) ,
        'DataType' => '2',
    );
    
    $datas['DataSign'] = kdn_encrypt($requestData, C('KDN_AppKey'));
