                ...
                if(!$Goods->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                   ...
                   // C('TOKEN_ON',true);                                                            
                    ...
                    if ($type == 2)
                    {
                        ...
                        $hg_price = $shop_price * (100-$rule);
                        $hg_price = $hg_price * 0.01;
                        $data['hg_price'] = $hg_price;
// print_r($data);die();                                                
                    ...
                    {         
                        ...
                        $data['hg_price'] = $hg_price;
                        $data['on_time'] = time();
                        $goods_id = $insert_id = $Goods->add($data); // 写入数据到数据库
                        $Goods->afterSave($goods_id);
                    }                                        
                    
                    $GoodsLogic->saveGoodsAttr($goods_id, $_POST['goods_type']); // 处理商品 属性

                    //2017.02.16 上传商品列表图片修改 添加的代码  以下
                    if(!empty(I("goods_id"))){
                        $delgoodsimgs = M('GoodsImages')->where('goods_id='.I("post.goods_id"))->delete();
                    }
                    
                    $imgs = I('post.goods_images');
                    // print_r($imgs);
                    // die();
                    $imgs = array_filter($imgs);
                    foreach ($imgs as $k => $val) {
                        $img['image_url'] = $val;
                        $img['goods_id'] = I('post.goods_id');
                        // print_r($img);
                        $addgoodsimgs = M('GoodsImages')->where('goods_id='.I("post.goods_id"))->add($img);
                    }
                    // die();
                    //2017.02.16 上传商品列表图片修改 添加的代码  以上
                    $return_arr = array(
                        'status' => 1,
                        'msg'   => '操作成功',                        
    ...
    public function introductions(){
        if(IS_POST){
            $data = I('post.');
            $data['type'] = 1;
            $result = M("introductions")->where("type=1")->find();
            if(empty($result)){
                $result = M("introductions")->add($data);
            }else{
                $result = M("introductions")->where("type=1")->save($data);
            }
        ...
        $introductions = M("introductions")->where("type=1")->find();
    ...
    public function article(){
        $info = M('article')->where('cat_id=2')->find();
        $data['title'] = I("title");
        ...
        if(I("act") == "add"){
            ...
            $res = M("article")->where("article_id=".I("article_id")." AND cat_id=2 ")->save($data);
            $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U('Admin/Goods');
            if($res){
                $this->success("操作成功",$referurl);
            }else{
    ...
    public function checkgoods(){
        ...
        if(IS_POST){
            $ids = I("ids");
            M("hg_goods")->where("hg_id = ".I("id"))->delete();
            foreach ($ids as $i => $is) {
                $data['goods_id'] = $ids[$i];
                $data['hg_id'] = I("id");
                $hg = M("hg")->where("id = ".I("id"))->find();
                $par_name = M("hg")->where("id = ".$hg['parentid'])->getField("zhuanqu");
                $data["hg_name"] = $par_name.$hg['zhuanqu'];
                M("hg_goods")->add($data);
            }
            echo json_encode($data);
        }else{
            $goods = M("goods")->field("goods_id,goods_name,original_img")->select();
            $hg_goods = M("hg_goods")->where("hg_id = ".I("id"))->select();
            foreach ($goods as $gd => $gds) {
                $goods_id[] = $gds['goods_id'];
                foreach ($hg_goods as $hgd => $hgds) {
                    if($gds['goods_id'] == $hgds['goods_id']){
                        $goods[$gd]['checked'] = "1";
                    }
                }
            }
