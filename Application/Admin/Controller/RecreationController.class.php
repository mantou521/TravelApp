...
class RecreationController extends BaseController {
    ...
    public function introductions(){
        if(IS_POST){
            $data = I('post.');
            $data['type'] = 6;
            $result = M("introductions")->where("type=6")->find();
            if(empty($result)){
                $result = M("introductions")->add($data);
            }else{
                $result = M("introductions")->where("type=6")->save($data);
            }
            $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U('Admin/Recreation');
            if($result){
        ...
        $introductions = M("introductions")->where("type=6")->find();
