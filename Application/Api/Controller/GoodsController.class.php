...
class GoodsController extends BaseController {
    ...
    public function goodsCategoryList(){        
        ...
        $goodsCategoryList = M('GoodsCategory')->where("parent_id = $parent_id AND is_show=1")->order("sort_order asc,parent_id_path desc")->select();
    ...
    public function goodsList(){
        ...
        $rule = I("post.rule");
        // if (!empty($rule)) {
        //     $filter_goods_id = M("goods")->where("rule=".$rule." and is_on_sale=1 and cat_id in(".  implode(',', $cat_id_arr).") ")->cache(true)->getField("goods_id",true);
        // }else{
        //     $filter_goods_id = M('goods')->where("is_on_sale=1 and cat_id in(".  implode(',', $cat_id_arr).") ")->cache(true)->getField("goods_id",true);
        // }

        if ($rule !== "all") {
            $where = "rule=".$rule." and is_on_sale=1 and cat_id in(".implode(',', $cat_id_arr).")";
            // print_r($where);
...
// $sql = M('goods')->getLastSql();echo $sql;
        // $filter_goods_id = M('goods')->where("is_on_sale=1 and cat_id in(".  implode(',', $cat_id_arr).") ")->cache(true)->getField("goods_id",true);
    	...
    	if($count > 0)
    	{
    		$goods_list = M('goods')->field('goods_id,cat_id,goods_sn,goods_name,market_price,shop_price,comment_count,original_img,hg_price,rule,is_free_shipping')->where("goods_id in (".  implode(',', $filter_goods_id).")")->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
    ...
    public function search(){
        ...
        $q  && ($_POST["q"] = $filter_param['q'] = $q); //加入帅选条件中
        if(empty($q))
    	...
    	if($count > 0)
    	{
            ...
            $goods_list = M('goods')->field('goods_id,cat_id,goods_sn,goods_name,market_price,shop_price,comment_count,original_img,hg_price,rule,is_free_shipping')->where("goods_id in (".  implode(',', $filter_goods_id).")")->order("$sort $sort_asc")->limit($page->firstRow)->select(); 
    		$filter_goods_id2 = get_arr_column($goods_list, 'goods_id');
    	}
    ...
    //搜索提示
    public function tjsearch($value='')
    {
        $q = I("q");
        $list = M('goods')->where("goods_name like '%{$q}%'  ")->getField("goods_name",true);
        $json_arr = array('status'=>1,'msg'=>'获取成功','result'=>$list );
        $json_str = json_encode($json_arr,TRUE); 
        exit($json_str);   
    }
    ...
    public function goodsInfo(){
        ...
        if($keys)
        {
             ...
             $result= array();
             foreach ($filter_spec as $key => $info) {
                // $typename = $info['spec_name'];
                // $typename = array_unique($typename);
                $result[$info['spec_name']][] = $info;
             } 
       ...
       // $goods['goods_content'] = str_replace('/Public/upload/', SITE_URL."/Public/upload/", $goods['goods_content']);
        ...
        $return['spec_goods_price']  = M('spec_goods_price')->where("goods_id = $goods_id")->getField("key,price,store_count,key_name"); // 规格 对应 价格 库存表
        // print_r($return['spec_goods_price']);
        $return['goods']['spec_key'] = "";
        $return['goods']['spec_key_name'] = "";
        foreach ($return['spec_goods_price'] as $sgp => $sgps) {
            if(!empty($return['spec_goods_price'][$sgp]['price']) && $goods['shop_price'] == $return['spec_goods_price'][$sgp]['price']){
                $return['goods']['spec_key'] = $return['spec_goods_price'][$sgp]['key'];
                $return['goods']['spec_key_name'] = $return['spec_goods_price'][$sgp]['key_name'];
                break;
            }
        }
        ...
        //是否收藏
        if(I("user_id")){
            $collects = M("goods_collect")->where("goods_id =" .$goods_id ." AND user_id = ".I("user_id")." AND shop = ".I("shop"))->find();
            if($collects){
                $return['goods']['collected'] = 1;
            }else{
                $return['goods']['collected'] = 0;
            }
        }else{
            $return['goods']['collected'] = 0;
        }
        ...
        //获取最近的两条评论
        $latest_comment = M('comment')->where("goods_id={$goods_id} AND is_show=1 AND parent_id=0")->limit(10)->select();
        $comment_num = M('comment')->where("goods_id={$goods_id} AND is_show=1 AND parent_id=0")->count('comment_id');

        $return['comment'] = $latest_comment ? $latest_comment : '';
        $return['comment_num'] = $comment_num ? $comment_num : '0';
    ...
    {        
        ...
        $pl_list = M('Comment')->where("goods_id=".$goods_id)->order("add_time desc")->limit($page->firstRow.','.$page->listRows)->select();
        ...
        //用户评论
        exit(json_encode(array('status'=>1,'msg'=>'获取成功','result'=>$list ,'replyList'=>$replyList )));
        // 管理员回复
        // exit(json_encode(array('status'=>1,'msg'=>'获取成功','result'=>$replyList )));
    }
    ...
    function collectGoods(){
        ...
        if($shop == 1){
            ...
            $count = M('Books')->where("books_id = $goods_id")->count();
        }
        if($count == 0)  exit(json_encode(array('status'=>1,'msg'=>'收藏商品不存在','result'=>array())));
        //删除收藏商品
        if($type==2){
            M('GoodsCollect')->where("user_id = {$this->user_id} and goods_id = $goods_id and shop = $shop")->delete();
            $res = $this->userLogic->get_goods_collect($this->user_id);
            exit(json_encode(array('status'=>1,'msg'=>'成功取消收藏','result'=>$res)));
        }
        $result = $this->userLogic->get_goods_collect($this->user_id);
        $count = M('GoodsCollect')->where("user_id = {$this->user_id} and goods_id = $goods_id and shop = $shop")->count();
        if($count > 0)        exit(json_encode(array('status'=>1,'msg'=>'您已收藏过该商品','result'=>$result )));
        $key = I("key");
        ...
        exit(json_encode(array('status'=>1,'msg'=>'收藏成功','result'=>$data)));
    }
    ...
    public function indexgoods(){
        // banner
        $uubanner = M("Banners")->where('local=2')->order("banners_id desc")->find();
        $uu_id = $uubanner['banners_id'];
        $uubans = M("Banners_images")->where('banners_id='.$uu_id)->order("banners_id desc")->select();
        foreach ($uubans as $u => $uu) {
            $uu_bans[] = $uu['image_url'];
        }

        $uutc1 = M("Banners")->where('local=3')->order("banners_id desc")->find();
        $uu_id_1 = $uutc1['banners_id'];
        $uutc_1 = M("Banners_images")->where('banners_id='.$uu_id_1)->order("banners_id desc")->select();
        foreach ($uutc_1 as $u_1 => $uu_1) {
            $uu_tc1[] = $uu_1['image_url'];
        }

        $uutc2 = M("Banners")->where('local=4')->order("banners_id desc")->find();
        $uu_id_2 = $uutc2['banners_id'];
        $uutc_2 = M("Banners_images")->where('banners_id='.$uu_id_2)->order("banners_id desc")->select();
        foreach ($uutc_2 as $u_2 => $uu_2) {
            $uu_tc2[] = $uu_2['image_url'];
        }

        //热门产品
        $hotgoods = M('Goods_')->where('is_hot=1')->order('goods_id desc')->limit("3")->select();
        
        // 海外换购专区
        $seasgoods['seasgoods_100'] = M('Goods_')->where("rule=100 and area=1")->order('goods_id desc')->limit("3")->select();//100换购商品
        $seasgoods['seasgoods_70'] = M('Goods_')->where("rule=70 and area=1")->order('goods_id desc')->limit("3")->select();//70换购商品
        $seasgoods['seasgoods_50'] = M('Goods_')->where("rule=50 and area=1")->order('goods_id desc')->limit("3")->select();//50换购商品
        $seasgoods['seasgoods_30'] = M('Goods_')->where("rule=30 and area=1")->order('goods_id desc')->limit("3")->select();//30换购商品

        //换购专区
        $buysgoods['buysgoods_100'] = M('Goods_')->where("rule=100 and area=2")->order('goods_id desc')->limit("7")->select();//100换购说商品
        $buysgoods['buysgoods_70'] = M('Goods_')->where("rule=70 and area=2")->order('goods_id desc')->limit("7")->select();//70换购说商品
        $buysgoods['buysgoods_50'] = M('Goods_')->where("rule=50 and area=2")->order('goods_id desc')->limit("7")->select();//50换购说商品
        $buysgoods['buysgoods_30'] = M('Goods_')->where("rule=30 and area=2")->order('goods_id desc')->limit("7")->select();//30换购说商品

        //特产专区specgoods['
        $specgoods['specgoods_100'] = M('Goods_')->where("rule=100 and area=3")->order('goods_id desc')->limit("7")->select();//100换购
        $specgoods['specgoods_70'] = M('Goods_')->where("rule=70 and area=3")->order('goods_id desc')->limit("7")->select();//70换购
        $specgoods['specgoods_50'] = M('Goods_')->where("rule=50 and area=3")->order('goods_id desc')->limit("7")->select();//50换购
        $specgoods['specgoods_30'] = M('Goods_')->where("rule=30 and area=3")->order('goods_id desc')->limit("7")->select();//30换购

        //自定义换购专区
        $customgoods = M('Goods_')->where("area=4")->order('goods_id desc')->limit("3")->select();//全部换购
        
        // 品牌logo
        $brands = M("Brand")->limit(20)->select();
         
        $json_arr = array('status'=>1,'msg'=>'获取成功','uu_bans'=>$uu_bans,'uu_tc1'=>$uu_tc1,'uu_tc2'=>$uu_tc2,'hotgoods'=>$hotgoods,'seasgoods'=>$seasgoods,'buysgoods'=>$buysgoods,'specgoods'=>$specgoods,'customgoods'=>$customgoods,'brand'=>$brands);
        $replace = array('src=&quot;\/'.SITE_URL.'\/Public','"'.SITE_URL.'\/Public');
        $search = array('src=&quot;\/Public','"\/Public');
        $json_str = json_encode($json_arr,TRUE); 
        $json_str = str_replace($search,$replace,$json_str);
    ...
    //热门产品更多
    public function hotgoods_m()
    {
        $hotgoods = M('Goods_')->where('is_hot=1')->order('goods_id desc')->select();
        $json_arr = array('status'=>1,'msg'=>'获取成功','hotgoods'=>$hotgoods);
        $this->replace($json_arr); 
    }

    //自定义换购更多
    public function customgoods_m()
    {
        $customgoods = M('Goods_')->where("area=4")->order('goods_id desc')->select();//
        $json_arr = array('status'=>1,'msg'=>'获取成功','hotgoods'=>$customgoods);
        $this->replace($json_arr); 
    }
    ...
    public function caifu(){
        // banner
        $uubanner = M("Banners")->where('local=1')->order("banners_id desc")->find();
        $uu_id = $uubanner['banners_id'];
        $uubans = M("Banners_images")->where('banners_id='.$uu_id)->order("banners_id desc")->select();
        // UU头条
        $first_title = M('Article')->field('article_id,title')->order("article_id desc")->select();
        //导航
        $daohang = M("introductions")->order("id asc")->select();
        $json_arr = array('status'=>1,'msg'=>'获取成功','uubans'=>$uubans,'first_title'=>$first_title,'daohang'=>$daohang);
    ...
    {
        $banner = M("Banners")->where('local=5')->order("banners_id desc")->find();
        $b_id = $banner['banners_id'];
        $bookbans = M("Banners_images")->where('banners_id='.$b_id)->order("banners_id desc")->select();
        $subscription = M("Books")->order('sales_sum desc')->limit(3)->select();
        $books = M("Books")->select();
        // print_r($books);

        $json_arr = array('status'=>1,'msg'=>'获取成功','bookbans'=>$bookbans,'subscription'=>$subscription,'books'=>$books);
        $this->replace($json_arr); 
    }
    // UU册详情
    ...
    {
        $books_id = I("books_id");
        $books = M("Books")->where('books_id='.$books_id)->find();
        $data['click_count'] = ++$books['click_count'];
        M("Books")->where('books_id='.$books_id)->save($data);
        $books_imgs = M("Books_images")->where('books_id='.$books_id)->select();
        foreach ($books_imgs as $b => $bi) {
            $images[] = $bi['image_url'];
        }

        $books['images'] = $images;
        $con = $books["keywords"];
        $where['keywords'] = array('like', "%".$con."%");
        $like = M("books")->where($where)->limit(3)->select();
        // $like = M("books")->where($where)->limit(3)->getField("books_id,books_name,market_price,shop_price,original_img");
        // $sql = M("books")->getLastSql();
        $result['books'] = $books;
        $result['like'] = $like;

        $json_arr = array('status'=>1,'msg'=>'获取成功','books'=>$result);

        $this->replace($json_arr); 
    }

    //UU册关于我们
    public function uubook_about(){
        $uuabout = M("Article")->where('cat_id=3')->select();
        $json_arr = array('status'=>1,'msg'=>'获取成功','uuabout'=>$uuabout,);

        $this->replace($json_arr); 
    }

    public function booksmore()
    {
        if(I("more") == "1"){
            $books = M("Books")->order('sales_sum desc')->select();
        }else if(I("more") == "2"){
            $books = M("Books")->select();
        }
        
        $json_arr = array('status'=>1,'msg'=>'获取成功','books'=>$books);
        $this->replace($json_arr); 
    }
