...
use Api\Logic\BooksLogic;
use Think\Page;
class CartbooksController extends BaseController {
    
     public $cartbooksLogic; // 购物车逻辑操作类
    /**
     * 析构流函数
     */
    public function  __construct() {   
        parent::__construct();                
        $this->cartbooksLogic = new \Home\Logic\CartbooksLogic();                     
    ...
    function addCart()
    {
        $books_id = I("books_id"); // 商品id
        $books_num = I("books_num");// 商品数量
        $books_spec = I("books_spec"); // 商品规格                
        $books_spec = json_decode($books_spec,true); //app 端 json 形式传输过来
        $unique_id = I("unique_id"); // 唯一id  类似于 pc 端的session id
        //$user_id = I("user_id",0); // 用户id        
        $result = $this->cartbooksLogic->addCart($books_id, $books_num, $books_spec,$unique_id,$this->user_id); // 将商品加入购物车
    ...
    {       
        ...
        // 查找购物车数量
        $unique_id = I("unique_id"); // 唯一id  类似于 pc 端的session id
        $cart_count =  cart_books_num(0,$unique_id);
    ...
    {                    
        $cart_form_data = $_POST["cart_form_data"]; // books_num 购物车商品数量
...
//        $user_id = I("user_id",0); // 用户id                
        ...
        $cartList = M('Cart')->where($where)->getField("id,books_num,selected"); 
        if($cart_form_data)
        {
            // 修改购物车数量 和勾选状态
            foreach($cart_form_data as $key => $val)
            {   
                $data['books_num'] = $val['booksNum'];
                $data['selected'] = $val['selected'];
                $cartID = $val['cartID'];
                if(($cartList[$cartID]['books_num'] != $data['books_num']) || ($cartList[$cartID]['selected'] != $data['selected'])) 
                    M('Cart')->where("id = $cartID")->save($data);
            }
            //$this->assign('select_all', $_POST['select_all']); // 全选框
        }                  
                       
        $result = $this->cartbooksLogic->cartList($this->user, $unique_id,1,0); // 选中的商品        
    ...
    public function cart2()
    {
        ...
        if($this->cartbooksLogic->cart_count($this->user_id,1) == 0 ) exit(json_encode (array('status'=>-2,'msg'=>'你的购物车没有选中商品','result'=>'')));
        
        // 购物车商品                    
        $cart_result = $this->cartbooksLogic->cartList($this->user, $unique_id,1,1); // 获取购物车商品          
        // 没有选中的不传递过去
        $cartList = array();
...
//                           'couponList'  =>$couponList, //优惠券列表@h
    ...
    public function cart3(){
        ...
        if($this->cartbooksLogic->cart_count($this->user_id,1) == 0 ) exit(json_encode(array('status'=>-1,'msg'=>'你的购物车没有选中商品','result'=>null))); // 返回结果状态
	...
	$order_books = M('cart')->where("user_id = {$this->user_id} and selected = 1")->select();
        $result = calculate_price($this->user_id,$order_books,$shipping_code,0,$address[province],$address[city],$address[district],$pay_points,$user_money,$coupon_id,$couponCode);
	...
	// 订单满额优惠活动		                
        ...
        $car_price = array(
            ...
            'booksFee'     => $result['result']['books_price'],// 商品价格            
        ...
        {            
            ...
            $result = $this->cartbooksLogic->addOrder($this->user_id,$address_id,$shipping_code,$invoice_title,$coupon_id,$car_price); // 添加订单                        
            exit(json_encode($result));            
        }
