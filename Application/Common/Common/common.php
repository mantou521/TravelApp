...
function queryExpress($postcom , $getNu){
	if(isset($postcom) && isset($getNu)){
		...
		$AppKey='19e73e201a28da53';

		$url ='http://api.kuaidi100.com/api?id='.$AppKey.'&com='.$postcom.'&nu='.$getNu.'&show=0&muti=1&order=desc';
...
/**
 * 保存微信app支付订单信息
 * @param $pay_arr   app支付订单信息
 */
    function add_wxpaylog($pay_arr){
        $pay_arr['add_time'] = time();
        M('wxpaylog')->add($pay_arr);
    }
...
{    
    ...
    if(empty(I("goods_id"))){
        $goods_arr = M('goods')->where("goods_id in(".  implode(',',$goods_id_arr).")")->getField('goods_id,weight,market_price,is_free_shipping,shipping_price'); // 商品id 和重量对应的键值对
        ...
        {       
            if(!array_key_exists('member_goods_price',$val))  
            {
                ...
                $order_goods[$key]['goods_price'] = $val['goods_price'] = $val['goods_price'] * $user['discount'];
            }
            //如果商品不是包邮的
            ...
            if($order_goods[$key]['shop'] == 2)  {
                $goods_price['uc'] += $order_goods[$key]['goods_price']*$val['goods_num']; // 商品总价
                $uc_shipping += $val['shipping_price'];
            }elseif($order_goods[$key]['shop'] == 1)  {
                $goods_price['hg'] += $order_goods[$key]['goods_price']*$val['goods_num']; // 商品总价
                $hg_shipping += $val['shipping_price'];
                // print_r($val['shipping_price']);
            }
        ...
        $shop = $shop[0];
        if($shop == 2){
            $goods_arr = M('books')->where("books_id=".I("goods_id"))->getField('books_id,weight,market_price,is_free_shipping,shipping_price'); // 商品id 和重量对应的键值对
        }elseif ($shop == 1) {
            $goods_arr = M('goods')->where("goods_id=".I("goods_id"))->getField('goods_id,weight,market_price,is_free_shipping,shipping_price'); // 商品id 和重量对应的键值对
        }
        foreach($order_goods as $key => $val)
        {       
            // if(!array_key_exists('member_goods_price',$val))  
            ...
            if($shop == 2){
                //如果商品不是包邮的
                if($goods_arr[$val['books_id']]['is_free_shipping'] == 0)
                    $goods_weight += $goods_arr[$val['books_id']]['weight'] * $val['goods_num']; //累积商品重量 每种商品的重量 * 数量
                $order_goods[$key]['goods_fee'] = I('goods_num') * $val['shop_price'];    // 小计            
                $order_goods[$key]['store_count']  = getGoodNum($val['books_id'],$val['spec_key']); // 最多可购买的库存数量 
                $shipping_price += $val['shipping_price'];
            }elseif ($shop == 1) {
                //如果商品不是包邮的
                if($goods_arr[$val['goods_id']]['is_free_shipping'] == 0)
                ...
                $shipping_price += $val['shipping_price'];
                    ...
                    // print_r($val);                        
            $goods_price += $order_goods[$key]['shop_price']; // 商品总价
            // print_r($goods_price);
            $cut_fee     += $val['goods_num'] * $val['market_price'] - $val['goods_num'] * $val['goods_price']; // 共节约
            $anum        += $val['goods_num']; // 购买数量
        }        
   ...
   //          $anum        += $val['goods_num']; // 购买数量
        ...
        // print_r($order_goods[$key]);die();
        if(empty(I("goods_id"))){
            // if($order_goods[$key]['shop'] == 2)  {
            //     $order_amount['uc'] = $goods_price['uc'] + $shipping_price - $coupon_price; // 应付金额 = 商品价格 + 物流费 - 优惠券
            // }elseif($order_goods[$key]['shop'] == 1)  {
            //     $order_amount['hg'] = $goods_price['hg'] + $shipping_price - $coupon_price; // 应付金额 = 商品价格 + 物流费 - 优惠券
            // }
            $order_amount = $goods_price['uc'] + $goods_price['hg'] + $shipping_price - $coupon_price;
        }else{
            $order_amount = $goods_price*I("goods_num") + $shipping_price - $coupon_price;
        }
       ...
       // $order_amount = $order_amount - $user_money; //  余额支付抵应付金额
        ...
        if($order_goods[$key]['shop'] == 2)  {
            $total_amount = $goods_price['uc'] + $uc_shipping;
           //订单总价  应付金额  物流费  商品总价 节约金额 共多少件商品 积分  余额  优惠券
        }elseif($order_goods[$key]['shop'] == 1)  {
            $total_amount = $goods_price['hg'] + $hg_shipping;
           //订单总价  应付金额  物流费  商品总价 节约金额 共多少件商品 积分  余额  优惠券
        }
       if(empty(I("goods_id"))){
            $shipping  = array('hg' => $hg_shipping,'uc' => $uc_shipping, );
       }
